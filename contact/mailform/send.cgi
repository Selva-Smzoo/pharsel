...
#04.sendmailのパス(サーバ会社へお問い合わせください)
$conf{'sendmail'} = '/usr/sbin/sendmail';

#06.設置者のアドレス(カンマ区切り)
$conf{'mailto'} = 'shimizu@selva-i.co.jp';

#07.送信完了時にリダイレクトするサンクスページ
$conf{'thanks'} = '../thanks.html';

#08.設置者に届くメールの件名
$conf{'subject'} = 'お問い合せがありました【株式会社セルバ】';

#09.送信者に届くメールの件名
$conf{'res_subject'} = 'お問い合せありがとうございます【株式会社セルバ】';

#10.送信者に届くメールの本文
$conf{'res_body'} = <<'__res_body_eof__';
この度は、お問い合せいただき、誠にありがとうございます。
